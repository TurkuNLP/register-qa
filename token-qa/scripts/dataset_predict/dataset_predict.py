...
def argparser():
    ...
    ap.add_argument('--dataset', metavar='FILE', required=True,
                    help='dataset for which texts to be predicted') #could also be string?
    ap.add_argument('--text_name', default="content", type=str, help="which field to use as the text to predict on")
    ...
    ap.add_argument('--start', default=0, metavar='INT', type=int,
                    help='the row in the dataset from which to start')
...
def load_text_batch(texts_dataset, ids_dataset, options, begin, end):
    batch = texts_dataset[begin:end]
    #print(batch[0])
    batch = [d[options.text_name] for d in batch]
     # have to somehow save where the current beginning and end is
    
     # put the amount here
    ids = ids_dataset[begin:end] 
    ids = [d[options.id_col_name] for d in ids]
    begin = end
    end = end +  options.batch_size 

    return ids, batch, begin, end
...
def main():
    ...
    log("loading dataset")
    dataset_stream = datasets.load_dataset(options.dataset, streaming=True, split="train",)
    #dataset = dataset_stream.take(1000000) # this takes 100 000 first without downloading the huge ass dataset, that was fast, now taking 1 million
    # take from between
    end = options.start+1000000
    dataset = dataset_stream.take(end) 
    dataset = dataset.skip(options.start)

    texts_dataset = list(dataset.select_columns(options.text_name))
    ids_dataset = list(dataset.select_columns(options.id_col_name))
    log("..done")

    with open(options.output, 'w') as outfile:
        
        ids, text_batch, begin, end = load_text_batch(texts_dataset, ids_dataset, options, 0, options.batch_size)
        ...
        while text_batch:
            ...
            ids, text_batch, begin, end = load_text_batch(texts_dataset, ids_dataset, options, begin, end)
